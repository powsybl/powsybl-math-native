...
/**
 * Gauss-Newton least squares minimization using CHOLMOD.
 *
 * <p>The tests use the buffer API the way a real Gauss-Newton outer loop should:
 * the direct buffers are allocated <em>once</em> (dimensions are stable across
 * iterations) and only their contents are refreshed each iteration via
 * {@link NativeBuffers#copyFrom}, with the result read back via
