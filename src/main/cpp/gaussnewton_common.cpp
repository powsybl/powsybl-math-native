...
void NormalEquations::buildPattern(int n, int m, const int* htP, const int* htI) {
    ...
    // is structurally present whenever parameter k appears in at least one
    // equation (row of H). If a column k of H is structurally empty we throw
    ...
    for (int j = 0; j < n; ++j) {
        ...
        if (diagIdx_[j] < 0) {
            throw std::runtime_error("NormalEquations: column " + std::to_string(j)
                                     + " of Ht is structurally empty - parameter appears in no"
                                       " equation, LM damping has no diagonal to modify");
        }
    }
